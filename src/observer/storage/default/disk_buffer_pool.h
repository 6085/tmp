...
#define BP_PAGE_SIZE (2048 << 10) // 2m
#define BP_PAGE_DATA_SIZE (BP_PAGE_SIZE - sizeof(PageNum))
#define BP_FILE_SUB_HDR_SIZE (sizeof(BPFileSubHeader))
#define MAX_PAGE_NUM ((BP_PAGE_DATA_SIZE - BP_FILE_SUB_HDR_SIZE) * 8)
#define BP_BUFFER_SIZE 500 // buffer number
#define MAX_OPEN_FILE 1024

typedef struct {
  PageNum page_num;
  int page_type; // 0 for meta, and 1 for data
  char data[BP_PAGE_DATA_SIZE];
} Page;
// sizeof(Page) should be equal to BP_PAGE_SIZE

typedef struct {
  PageNum total_page_count; // only the first page contains this information
  PageNum page_count;
  int allocated_pages;
  int start_index;
} BPFileSubHeader;
...
public:
  ...
  PageNum last_allocate_meta_page_; // 上次从那个元数据页面分配的数据
} ;
...
protected:
  ...
  RC allocate_page_from_sub(int file_id, BPFileHandle *file_handle, 
														BPPageHandle *sub_hdr_page_handle, BPFileSubHeader *file_sub_header, 
