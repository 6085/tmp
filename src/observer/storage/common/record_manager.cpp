...
RC RecordPageHandler::init(DiskBufferPool &buffer_pool, int file_id, PageNum page_num) {
  if (disk_buffer_pool_ != nullptr) {
    LOG_WARN("Disk buffer pool has been opened(fileid:pagenum %d:%s) now want to open file_id:page_num %d:%d.",
             file_id_, page_handle_.frame->page.page_num, file_id, page_num);
...
RC RecordFileHandler::insert_record(const char *data, int record_size, RID *rid) {
  ...
  for (int i = 0; i < page_count; i++) {
    ...
    if (record_page_handler_.bp_page_handle().frame->page.page_type != 1) // data page
    {
      continue;
    }
    ...
    if (!record_page_handler_.is_full()) {
      ...
      break; // TODO 特定优化，不再重新找，直接分配新的页面
    }
  }
...
RC RecordFileScanner::get_next_record(Record *rec) {
  ...
  while (current_record.rid.page_num < page_count) {
    ...
    if (current_record.rid.page_num != record_page_handler_.get_page_num()) {
      ...
      if (record_page_handler_.bp_page_handle().frame->page.page_type != 1) { // not a data page
