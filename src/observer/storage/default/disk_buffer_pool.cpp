...
RC DiskBufferPool::create_file(const char *file_name)
{
  ...
  fileSubHeader->total_page_count = 1;
...
RC DiskBufferPool::allocate_page_from_sub(int file_id, BPFileHandle *file_handle, BPPageHandle *sub_hdr_page_handle, BPFileSubHeader *file_sub_header, BPPageHandle *page_handle)
{
  RC rc;
  file_handle->last_allocate_meta_page_ = sub_hdr_page_handle->frame->page.page_num;

  int byte = 0, bit = 0;
  char *bitmap = (char*)(file_sub_header) + BP_FILE_SUB_HDR_SIZE;
  if ((file_sub_header->allocated_pages) < (file_sub_header->page_count)) {
    // There is one free page
    for (int i = 0; i < file_sub_header->page_count; i++) {
      byte = i / 8;
      bit = i % 8;
      if (((bitmap[byte]) & (1 << bit)) == 0) {
        (file_sub_header->allocated_pages)++;
        bitmap[byte] |= (1 << bit);
        return get_this_page(file_id, i + file_sub_header->start_index, page_handle);
      }
    }
  }

  if ((rc = allocate_block(&(page_handle->frame))) != RC::SUCCESS) {
    LOG_ERROR("Failed to allocate page %s, due to no free page.", file_handle->file_name);
    return rc;
  }

  file_handle->file_sub_header->total_page_count++;
  PageNum page_num = file_sub_header->page_count;
  file_sub_header->allocated_pages++;
  file_sub_header->page_count++;

  byte = page_num / 8;
  bit = page_num % 8;
  bitmap[byte] |= (1 << bit);
  // file_handle->hdr_frame->dirty = true;
  sub_hdr_page_handle->frame->dirty = true;

  memset(&(page_handle->frame->page), 0, sizeof(Page));
  page_handle->frame->page.page_type = 1;
  page_handle->frame->page.page_num = file_sub_header->page_count - 1 + file_sub_header->start_index;
  page_handle->frame->dirty = false;
  page_handle->frame->file_desc = file_handle->file_desc;
  page_handle->frame->pin_count = 1;
  page_handle->frame->acc_time = current_time();
  ...
  if ((rc = flush_block(page_handle->frame)) != RC::SUCCESS) {
...
RC DiskBufferPool::allocate_page(int file_id, BPPageHandle *page_handle)
{
  ...
  PageNum total_page_num = file_handle->file_sub_header->total_page_count;
  
  // find a non-full meta-page
  for (PageNum i = file_handle->last_allocate_meta_page_; i < total_page_num; i += MAX_PAGE_NUM + 1) {
    BPPageHandle sub_page_handle;
    tmp = get_this_page(file_id, i, &sub_page_handle);
    if (tmp != RC::SUCCESS){
      LOG_ERROR("Failed to get sub meta page. rc=%s", strrc(tmp));
      return tmp;
    }

    BPFileSubHeader *file_sub_header = (BPFileSubHeader*)(sub_page_handle.frame->page.data);
    if (file_sub_header->allocated_pages < file_sub_header->page_count) {
      return allocate_page_from_sub(file_id, file_handle, &sub_page_handle, file_sub_header, page_handle);
    }
  }

  // cannot find meta-page, alloc a new one
  BPPageHandle sub_page_handle;
  if ((tmp = allocate_block(&(sub_page_handle.frame))) != RC::SUCCESS) {
    LOG_ERROR("Failed to allocate page %s, due to no free page.", file_handle->file_name);
    return tmp;
  }

  memset(&sub_page_handle.frame->page, 0, BP_PAGE_SIZE);
  ...
  file_sub_header->allocated_pages++;
  file_sub_header->page_count++;
  file_sub_header->start_index = file_handle->file_sub_header->total_page_count - 1;
  ...
  bitmap[byte] |= (1 << bit);
  // file_handle->hdr_frame->dirty = true;
  sub_page_handle.frame->dirty = true;
  sub_page_handle.frame->acc_time = current_time();
  sub_page_handle.frame->pin_count = 0;
  sub_page_handle.frame->file_desc = file_handle->file_desc;
  sub_page_handle.frame->page.page_type = 0;
  sub_page_handle.frame->page.page_num = file_handle->file_sub_header->total_page_count - 1;

  tmp = allocate_page_from_sub(file_id, file_handle, &sub_page_handle, file_sub_header, page_handle);
  if (tmp == RC::SUCCESS && page_handle->frame->page.page_num <= 0) {
    LOG_ERROR("allocate invalid page. page num<=0 %d", page_handle->frame->page.page_num);
  }
  return  tmp;
}
...
RC DiskBufferPool::get_page_count(int file_id, int *page_count)
{
  ...
  *page_count = open_list_[file_id]->file_sub_header->total_page_count;
  return RC::SUCCESS;
}

RC DiskBufferPool::check_page_num(PageNum page_num, BPFileHandle *file_handle)
{
  if (page_num >= file_handle->file_sub_header->total_page_count) {
    LOG_ERROR("Invalid pageNum:%d, total_page_count=%d, file's name:%s", 
      page_num, file_handle->file_sub_header->total_page_count, file_handle->file_name);
